...
if (isset($_POST['submitsignin']))  {
    ...
    //  $count = mysqli_num_rows($result);
     ...
     //   $_SESSION['referalcode']=$result[0]['referalcode'];
        ...
        $code = random_strings(6);
         $currentdate = date("Y-m-d h:i:s");       
            ...
            $sql =$conn->prepare("INSERT INTO `user` ( `name`,
                `email`, `password`,`created`,`usertype`,`referalcode`) VALUES ('$name',
                '$email','$pass','$currentdate','$usertype','$code')");
  $result =    $sql->execute();
...
$id=$_SESSION['id'];
    ...
    if (isset($_POST['referal'])) {
      ...
      $sqled = $conn->prepare("Select * from user where referalcode='$referal'");
      $sqled->execute();
      $res =  $sqled->fetchAll(PDO::FETCH_ASSOC);
      $numer =$sqled->rowCount();
      $refereduser=$res[0]['id'];
       if($numer>0) {

        $sql =$conn->prepare("INSERT INTO `referalwallet` ( `points`,
        `userid`, `refereduserid`,`created`,`type`) VALUES ('1',
        '$id','$refereduser','$currentdate','credit')");
          $result =    $sql->execute();
    }
  }
